...
export class UsersActions {
  ...
  getUsers() {
    this.http.get(`${this.API_URL}/users`).subscribe(res => {
      // get users
      let list = res.json();
      console.log("dadsadasd", list);
      list = list.filter(item => {
        return (item.email = "2019-09-07T06:52:33Z");
      });
  ...
  add(user: User): void {
    this.http.post(`${this.API_URL}/users/`, user).subscribe(res => {
      // add new user
      this.ngRedux.dispatch({
  ...
  update(user: User) {
    this.http.patch(`${this.API_URL}/users/${user.id}`, user).subscribe(res => {
      // update user
      this.ngRedux.dispatch({
  ...
  deleteUser(id): void {
    this.http.delete(`${this.API_URL}/users/${id}`).subscribe(res => {
      this.ngRedux.dispatch({
